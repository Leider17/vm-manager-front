<template>
    <div class="border border-gray-300 px-7 py-4 rounded-2xl shadow-2xl md:w-6/12 lg:w-4/12 md:h-107 flex flex-col items-center justify-center">
        <h2 class="text-center font-bold text-2xl font-serif mb-10">{{ mode === 'register' ? 'REGISTRAR SU CUENTA' : 'INICIAR SESIÓN' }}</h2>
        <form class ="flex flex-col gap-1 items-end h-full" @submit.prevent="handleSubmit">

            <div class="flex gap-1 items-center flex-col md:flex-row" v-if="mode === 'register'">
            ...
            <div class="h-5 w-full">
                <div v-if="v$.name.$error && v$.name.$dirty" class="flex justify-center">
            ...
            <div class="h-5 w-full">
                <div v-if="v$.email.$error && v$.email.$dirty" class="flex justify-center">
            ...
            <div class="flex gap-1 items-center flex-col md:flex-row">
                ...
                <div class="relative">
                    <InputForm :type="showPassword ? 'text' : 'password'" id="password" v-model="v$.password.$model" placeholder="Ingrese su contraseña" required />
                    <button type="button" class="absolute right-3 top-2.5 hover:cursor-pointer" @click="showPassword = !showPassword">
                        <img :src="showPassword ? 'images/eye-off.png' : 'images/eye-on.png'" alt="toggle password visibility" class="w-5 h-5"/>
                    </button>
                </div>
            </div>
            <div class="h-10 w-full">
                <div v-if="(v$.password.$error && v$.password.$dirty) || authStore.error" class="flex justify-center">
                    ...
                    <MessageError v-if="authStore.error">{{ authStore.error }}</MessageError>
                </div>
            </div>

            
            <div class="w-full flex flex-col items-center">
                <Button type="submit" :class="'h-13 '">{{ mode === 'register' ? 'REGISTRATE' : 'INGRESAR' }}</Button>
            </div>
        </form>
